    ...
    <div className="min-h-screen flex flex-col pt-8 p-6 max-w-7xl mx-auto space-y-10 animate-slide-up">
      ...
      <header className="flex items-center justify-between bg-surface-900/50 p-4 rounded-3xl border border-surface-800 backdrop-blur-md sticky top-4 z-40">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-display font-black text-surface-50 tracking-tight">TERABOUND <span className="text-brand-500">HUB</span></h1>
            {isTenantSelected && (
           ...
           <div className="card p-6 space-y-6">
              <div className="flex flex-col items-center text-center space-y-4">
                 <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-surface-800 to-surface-700 border-2 border-surface-700 flex items-center justify-center overflow-hidden shadow-xl">
                    {context?.user.photoURL ? <img src={context.user.photoURL} alt="Avatar" className="w-full h-full object-cover" /> : <div className="text-3xl font-black text-surface-400">TB</div>}
                 </div>
                 <div>
                    <h2 className="text-base font-bold text-surface-100">{context?.user.displayName}</h2>
                    <span className="px-2 py-0.5 rounded bg-brand-500/10 text-brand-400 text-[9px] font-black uppercase tracking-widest border border-brand-500/10">Cuenta Global</span>
                 </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {context?.availableTenants && context.availableTenants.length > 0 ? (
                         ...
                         <div className="p-6 flex items-center justify-between">
                            <div className="flex items-center gap-5">
                               <div className="w-12 h-12 rounded-2xl bg-surface-950 border border-surface-800 flex items-center justify-center group-hover:border-brand-500/30 transition-colors">
                                  <Building2 className={`w-6 h-6 transition-colors ${selecting === tenant.id ? 'text-brand-500' : 'text-surface-600 group-hover:text-brand-500'}`} />
                               </div>
                               <div>
                                  <h4 className="text-base font-bold text-surface-100 uppercase tracking-tight">{tenant.legalName}</h4>
                                  <span className="text-[9px] bg-brand-500/10 text-brand-400 px-2 py-0.5 rounded font-black uppercase tracking-tighter border border-brand-500/20">{tenant.roleName}</span>
                               </div>
                            </div>
                    ...
                    <div className="col-span-full bg-surface-900/30 border-2 border-dashed border-surface-800 rounded-3xl p-16 flex flex-col items-center justify-center text-center space-y-4">
                       <Plus className="w-12 h-12 text-surface-800" />
                       <div className="max-w-xs">
                          <p className="text-sm font-bold text-surface-300">Sin empresas asignadas</p>
                          <p className="text-xs text-surface-600 mt-2">Contacta a soporte para que te asigne una membresía en una organización activa.</p>
                       </div>
                    </div>
                  )}
                </div>
             ...
             <div className="space-y-8 animate-fade-in">
                <div className="flex items-center justify-between px-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-surface-500 font-bold uppercase tracking-widest italic">Hub Launcher v1.0 — {context.tenant.legalName}</p>
                  </div>
                   ...
                   {context.modules && context.modules.length > 0 ? (
                     context.modules.map((module: any) => (
                       ...
                       >
                          ...
                          <div className="relative flex flex-col items-center justify-center p-8 bg-surface-900 border border-surface-800 rounded-3xl group-hover:border-brand-500/50 group-hover:bg-surface-800/50 transition-all aspect-square">
                             ...
                             <h4 className="text-sm font-black text-surface-100 uppercase tracking-tight text-center group-hover:text-brand-400 transition-colors">{module.name}</h4>
                             <p className="text-[9px] text-surface-600 font-mono mt-2 uppercase opacity-0 group-hover:opacity-100 transition-opacity">Ver Módulo</p>
                     ...
                     <div className="col-span-full bg-surface-900/30 border-2 border-dashed border-surface-800 rounded-3xl p-16 flex flex-col items-center justify-center text-center space-y-4">
                        ...
                        <div className="max-w-xs">
                           ...
                           <p className="text-xs text-surface-600 mt-2">Tu empresa aún no ha activado ningún módulo de negocio o no tienes los permisos necesarios.</p>
                        </div>
                     </div>
                   )}

                   {/* Admin shortcut if user is PlatformAdmin */}
                   {context.user.globalType === 'admin' && (
                     ...
                     >
                        ...
                        <div className="relative flex flex-col items-center justify-center p-8 bg-surface-900 border border-surface-800 border-dashed rounded-3xl group-hover:border-blue-500/50 group-hover:bg-blue-500/5 transition-all aspect-square">
                           ...
                           <h4 className="text-sm font-black text-surface-100 uppercase tracking-tight text-center group-hover:text-blue-400 transition-colors">Sistema Admin</h4>
                        </div>
                     </div>
                   )}
                </div>
